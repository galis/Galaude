...
会话会自动存盘到 `sessions/<id>.json`，可随时 `--continue` / `--resume` 接着聊。
...
src/
  ...
  store.ts    # 会话持久化（sessions/<id>.json：存盘/读取/列出/最近）
  agent.ts    # 核心 think→act→observe 循环 + 流式 + 会话(Session) + 持久化；对外发「事件」
  ui.tsx      # Ink 交互界面（固定底部输入框 + 上方滚动 + 斜杠菜单 + 历史/光标）
